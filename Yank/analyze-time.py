...
def main(source_directory,step, verbose=False): 
    ...
    for suffix in suffixes:
        fullpathcheck= os.path.join(source_directory, 'complex-%s.nc' %(suffix))
        if (not os.path.exists(fullpathcheck)): continue
        ncfilecheck=netcdf.Dataset(fullpathcheck,'r')
    ...
    for x in range(100,niterationscheck+step,step):
        for phase in phase_prefixes:
            ...
            # Estimate free energies.
                (Deltaf_ij, dDeltaf_ij) = estimate_free_energies(ncfile, ndiscard = nequil,nuse=x)

            # Estimate average enthalpies
                (DeltaH_i, dDeltaH_i) = estimate_enthalpies(ncfile, ndiscard = nequil, nuse=x)
    ...
    # Compute binding free energy.
        ...
        logger.info("")
        logger.info("Binding free energy until iteration %s: %16.3f +- %.3f kT (%16.3f +- %.3f kcal/mol)" % (str(x),DeltaF, dDeltaF, DeltaF * kT / units.kilocalories_per_mole, dDeltaF * kT / units.kilocalories_per_mole))
        logger.info("")
        #logger.info("DeltaG vacuum       : %16.3f +- %.3f kT" % (data['vacuum']['DeltaF'], data['vacuum']['dDeltaF']))
        logger.info("DeltaG solvent until iteration %s     : %16.3f +- %.3f kT" % (str(x),data['solvent']['DeltaF'], data['solvent']['dDeltaF']))
        logger.info("DeltaG complex until iteration %s     : %16.3f +- %.3f kT" % (str(x), data['complex']['DeltaF'], data['complex']['dDeltaF']))
        ...
        logger.info("Binding enthalpy until iteration %s   : %16.3f +- %.3f kT (%16.3f +- %.3f kcal/mol)" % (str(x),DeltaH, dDeltaH, DeltaH * kT / units.kilocalories_per_mole, dDeltaH * kT / units.kilocalories_per_mole))
